...
class TestEKFSLAM:
    """Tests para la clase EKFSLAM."""

    def setup(self):
        """Inicializa EKF-SLAM para tests."""
    ...
    def test_predict_straight(self):
        """Verifica predicción de movimiento recto."""
        self.setup()
        self.ekf.predict(v=1.0, omega=0.0, dt=1.0)
        # Debería moverse 1m en x (theta=0)
    ...
    def test_predict_turn(self):
        """Verifica predicción de giro."""
        self.setup()
        self.ekf.predict(v=0.0, omega=np.pi/2, dt=1.0)
        pose = self.ekf.robot_pose
        assert abs(pose[0] - 5.0) < 0.01  # No se mueve en x
        assert abs(pose[1] - 5.0) < 0.01  # No se mueve en y
        assert abs(pose[2] - np.pi/2) < 0.01  # Gira 90°

    def test_predict_increases_uncertainty(self):
        """La predicción debe aumentar la covarianza."""
        self.setup()
    ...
    def test_add_landmark(self):
        """Verifica que se agregan landmarks correctamente."""
        self.setup()
        # Observar un landmark a 3m, 0rad (delante del robot en theta=0)
        observations = [(3.0, 0.0)]  # rango=3, ángulo=0 (global)
        self.ekf.update(observations)
        assert self.ekf.num_landmarks == 1
        # El landmark debería estar en ~(8, 5)
        lm = self.ekf.landmarks[0]
        assert abs(lm[0] - 8.0) < 0.5
        assert abs(lm[1] - 5.0) < 0.5

    def test_state_dimension_grows(self):
        """El estado crece con cada nuevo landmark."""
        self.setup()
    ...
    def test_covariance_symmetry(self):
        """La covarianza debe ser simétrica."""
        self.setup()
    ...
    def test_data_association(self):
        """Verifica que landmarks cercanos se asocian correctamente."""
        self.setup()
        # Primer landmark
        self.ekf.update([(3.0, 0.0)])
        n_before = self.ekf.num_landmarks
        # Observar el mismo landmark de nuevo (similar posición)
        self.ekf.update([(3.0, 0.0)])
        # No debería agregar un nuevo landmark
        assert self.ekf.num_landmarks == n_before

    def test_new_landmark_far(self):
        """Un landmark lejano debe crear uno nuevo."""
        self.setup()
        self.ekf.update([(3.0, 0.0)])
        n_before = self.ekf.num_landmarks
        # Observar en dirección completamente diferente
        self.ekf.update([(5.0, np.pi)])
        assert self.ekf.num_landmarks == n_before + 1

    def test_step_method(self):
        """Verifica el método step (predict + update)."""
        self.setup()
    ...
    def test_multiple_steps_convergence(self):
        """Múltiples observaciones del mismo landmark reducen incertidumbre."""
        self.setup()
        # Observar un landmark fijo múltiples veces
        for _ in range(10):
        ...
        if self.ekf.num_landmarks > 0:
            ...
            # La covarianza debería ser pequeña después de muchas observaciones
            assert np.trace(lm_cov) < 1.0

    def test_normalize_angle(self):
        """Verifica normalización de ángulos."""
    ...
    def test_pose_covariance(self):
        """Verifica que get_pose_covariance retorna 3x3."""
        self.setup()
        cov = self.ekf.get_pose_covariance()
        assert cov.shape == (3, 3)

    def test_estimated_path_tracking(self):
        """Verifica que el historial de poses se actualiza."""
        self.setup()
        for i in range(5):
